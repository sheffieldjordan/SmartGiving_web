...
const GetEthereumGifts = async completion => {
  const recipientList = await factory.methods.getRecipients().call();
  getGiftsAndStats(recipientList, completion);
};

//gets a list of gifts for every recipient, and then gets stats on each Gift
async function getGiftsAndStats(recipientList, completion) {
...
module.exports = GetEthereumGifts;
